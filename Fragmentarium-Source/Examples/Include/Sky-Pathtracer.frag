	...
	for( int i=0; i <RayDepth; i++ )
	{
		if (trace(from,dir,hit,hitNormal)) {
			...
			if (rand() > reflectivity ) {
				...
				vec3 a;
				vec3 b;
				float sunLight = dot(hitNormal, sunDirection);
				if (sunLight>0.0 && !trace(hit+ sunDirection*3.0*minDist,sunDirection,a,b)) {
					direct += color*0.01*SunIntensity(sunDirection.z)*sunLight;
				}
			...
			return direct + color * (sunsky(dir, i>0));
		}
	}
	return direct;
}
